...
const formStyle = css` display: flex; flex-direction: column; gap: 2.5rem; `;
const inputGroupStyle = css` display: flex; flex-direction: column; gap: 0.6rem; `;
const labelStyle = css` font-size: 0.85rem; font-weight: 700; letter-spacing: 0.05em; color: ${colors?.primary || '#111111'}; `;
...
const noticeContainerStyle = css`
  ...
  gap: 6px;
  align-items: center;
  .sub-text { font-size: 0.85rem; color: ${colors?.gray?.[600] || '#666666'}; line-height: 1.4; margin: 0; strong { color: ${colors?.primary || '#111111'}; font-weight: 600; } }
  .accent-dot { color: ${colors?.accent || '#c5a47e'}; margin-right: 4px; font-weight: bold; }
  .sub-text-info { font-size: 0.78rem; color: ${colors?.gray?.[400] || '#aaaaaa'}; margin-top: 4px; }
`;
...
const fileCardStyle = css` 
  display: flex; align-items: center; gap: 12px; padding: 10px; border: 1px solid ${colors?.gray?.[200] || '#e5e5e5'}; border-radius: 6px; background-color: #fafafa; position: relative; 
  .img-thumbnail { width: 50px; height: 50px; border-radius: 4px; overflow: hidden; position: relative; img { width: 100%; height: 100%; object-fit: cover; } } 
  .file-info { 
    display: flex; flex-direction: column; overflow: hidden; padding-right: 20px; 
    .file-name { font-size: 0.85rem; font-weight: 500; color: #222222; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } 
    .file-size { font-size: 0.75rem; color: ${colors?.gray?.[400] || '#aaaaaa'}; } 
  } 
`;

const thumbnailBadgeStyle = css` position: absolute; bottom: 0; left: 0; width: 100%; background-color: rgba(197, 164, 126, 0.9); color: ${colors.white}; font-size: 0.55rem; font-weight: bold; text-align: center; padding: 2px 0; `;
...
const cancelButtonStyle = css`
  padding: 14px 28px; font-size: 0.95rem; font-weight: 700; color: #222222; background-color: transparent; border: 1.5px solid ${colors?.primary || '#9e0012'}; border-radius: 6px; cursor: pointer; transition: all 0.2s cubic-bezier(0.25, 1, 0.5, 1);
  &:hover { background-color: rgba(229, 0, 0, 0.04); box-shadow: 0 4px 12px rgba(229, 0, 0, 0.06); }
`;

const submitButtonStyle = (isSubmitting: boolean) => css` padding: 14px 35px; font-size: 0.95rem; font-weight: 600; color: ${colors.white}; background-color: ${colors?.primary || '#111111'}; border: none; border-radius: 6px; cursor: ${isSubmitting ? "not-allowed" : "pointer"}; opacity: ${isSubmitting ? 0.7 : 1}; `;

const modalOverlayStyle = css` position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(0, 0, 0, 0.4); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; z-index: 9999; `;
const modalBoxStyle = css` background-color: ${colors.white}; padding: 30px; border-radius: 12px; width: 100%; max-width: 420px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15); border: 1px solid ${colors?.gray?.[100] || '#f5f5f5'}; .modal-title { font-size: 1.2rem; font-weight: 700; color: ${colors?.primary || '#111111'}; margin-bottom: 8px; } .modal-desc { font-size: 0.88rem; color: ${colors?.gray?.[500] || '#666666'}; line-height: 1.5; margin-bottom: 20px; } `;
const modalInputStyle = css` width: 100%; padding: 12px 16px; font-size: 1rem; color: ${colors?.gray?.[800] || '#222222'}; border: 1.5px solid ${colors?.accent || '#c5a47e'}; border-radius: 6px; outline: none; letter-spacing: 0.3em; text-align: center; margin-bottom: 24px; `;
...
const modalSubmitBtnStyle = (isVerifying: boolean) => css` padding: 10px 24px; font-size: 0.9rem; font-weight: 600; color: ${colors.white}; background-color: ${colors?.primary || '#111111'}; border: none; border-radius: 6px; cursor: ${isVerifying ? "not-allowed" : "pointer"}; opacity: ${isVerifying ? 0.7 : 1}; `;